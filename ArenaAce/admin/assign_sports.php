...
if (isset($_POST['submit'])) {
    ...
    $sql = "INSERT INTO `assign_sport`(`arena_id`, `sport_id`) VALUES ('$arena_id','$sport_id')";

    // echo $sql; exit;

    if (mysqli_query($con, $sql) or die("query failed")) {
        header("Location: assign_sports.php");
    }
}

if (isset($_GET['delete_id'])) {
    $sid = $_GET['delete_id'];
    mysqli_query($con, "delete from assign_sport where id = '$sid'");
    echo "<script>alert('Data Deleted');</script>";
    echo "<script>window.location.href='assign_sports.php'</script>";
}
?>
                                                    ...
                                                    <div class="row mt-3">
                                                        ...
                                                        <div class="col-lg-4 mt-4">
                                                            <button type="submit" name="submit"
                                                                class="btn btn-primary">Assign Sports</button>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                        ...
                        <div class="col-lg-12">
                            <div class="row mt-3">
                                <div class="col">
                                    <?php
                                    include "config.php";
                                    $sql = "select assign_sport.id as sid,arena.name as arena,sport.name as sport from arena inner join assign_sport on (arena.a_id = assign_sport.arena_id) inner join  sport on (sport.s_id = assign_sport.sport_id )";
                                    $result = mysqli_query($con, $sql) or die("Query failed");
                                    if (mysqli_num_rows($result) > 0) {
                                        ?>
                                                ...
                                                <tbody>
                                                    ...
                                                    while ($row = mysqli_fetch_assoc($result)) {
                                                        ...
                                                        <tr>
                                                            ...
                                                            <td>
                                                                <a href="assign_sports.php?delete_id=<?php echo $row['sid'] ?>"
